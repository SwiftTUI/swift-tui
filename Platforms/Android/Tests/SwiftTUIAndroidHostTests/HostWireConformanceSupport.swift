  ...
  static func parseStep(
    ...
    case "emit":
      guard
        ...
        throw HostWireConformanceError.invalid("\(context): emit is inapplicable to \(entry.kind)")
      }
    ...
    case "drop":
      guard entry.kind == .record || entry.kind == .webPainter else {
        throw HostWireConformanceError.invalid("\(context): drop is inapplicable to \(entry.kind)")
      }
