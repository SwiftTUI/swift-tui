...
public enum SummaryReducer {
  public static func reduce(
    ...
    return PerfSummary(
      ...
      cpuSecondsPerInputEvent: ratio(totalCPUSeconds, Double(events.count)),
  ...
  private static func cancellationCount(_ frames: [PerfFrameRecord]) -> Int {
    max(
...
extension PerfFrameRecord {
  fileprivate var isCommitted: Bool {
    tailJobState == "completed" && !dropDecision.hasPrefix("drop")
  }
}
