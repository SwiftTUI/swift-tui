...
| WASI / browser | `SwiftTUIWASI` (`WASIRunner`) | Canvas (default), experimental DOM | Swift compiled to WASI. Raster output presented through the `web-surface` transport. Uses the stack-lean resolve profile by default. |
