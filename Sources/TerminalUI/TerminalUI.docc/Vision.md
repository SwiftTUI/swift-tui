...
Project philosophy, scope, and the reasoning behind which SwiftUI concepts ship today and which are deliberately deferred.
...
3. The deviation is documented here and reflected in the public API surface

When SwiftUI precedent and terminal-native practice disagree, TerminalUI should keep the SwiftUI-shaped authoring story only if it still produces a good terminal experience. If a direct translation would create page-like, mouse-centric, overdecorated, or otherwise non-native terminal behavior, the framework should reinterpret the behavior toward terminal-native defaults.
...
- **Terminal-native reinterpretation of defaults.** Automatic chrome, app shell composition, and navigation-oriented surfaces may intentionally diverge from desktop SwiftUI precedent when modern terminal UX has a clearly better answer.
...
Commands, keybindings, and scopes — `ActionScope`, `Panel`, `FocusContainment`, `.keyCommand`, `.paletteCommand`, `.toolbar`, and `.toolbarItem` — already follow this pattern: commands belong to **scopes**, a superset abstraction over navigation, sub-hierarchies, and modal modes, with focus-chain membership as the activation predicate.

Navigation-oriented surfaces that are especially central to terminal UX, such as `TabView` and split-pane workspace layouts composed from stacks, may land earlier than their desktop-GUI counterparts if they are expressed as a SwiftUI-shaped but terminal-native subset.
