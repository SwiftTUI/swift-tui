...
internal enum _PrismNumeric {
    ...
    @inlinable static func approxEqual(_ lhs: Double, _ rhs: Double, tolerance: Double = 1e-12) -> Bool {
...
public struct Matrix3x3: Hashable, Sendable, Codable {
    public let m11: Double; public let m12: Double; public let m13: Double
    public let m21: Double; public let m22: Double; public let m23: Double
    public let m31: Double; public let m32: Double; public let m33: Double
    ...
    public init(
        ...
        precondition(_PrismNumeric.isFinite(m11, m12, m13, m21, m22, m23, m31, m32, m33), "Matrix3x3 components must be finite")
...
public struct Chromaticity: Hashable, Sendable, Codable {
    ...
    public init(x: Double, y: Double) {
        ...
        precondition(x > 0 && y > 0 && x + y <= 1.0 + 1e-12, "Chromaticity must satisfy x > 0, y > 0, x + y <= 1")
        self.x = x
        self.y = y
    }
...
public struct ReferenceWhite: Hashable, Sendable, Codable {
    ...
    public init(name: String, x: Double, y: Double) {
        ...
        precondition(x > 0 && y > 0 && x + y <= 1.0 + 1e-12, "Reference white must satisfy x > 0, y > 0, x + y <= 1")
        self.name = trimmed
        self.x = x
        self.y = y
    }
...
public extension ReferenceWhite {
    static let d50 = ReferenceWhite(name: "D50", x: 0.34567, y: 0.35850)
    static let d55 = ReferenceWhite(name: "D55", x: 0.33242, y: 0.34743)
    static let d60 = ReferenceWhite(name: "D60", x: 0.32168, y: 0.33767)
    static let d65 = ReferenceWhite(name: "D65", x: 0.31270, y: 0.32900)
    static let d75 = ReferenceWhite(name: "D75", x: 0.29903, y: 0.31488)
    static let e = ReferenceWhite(name: "E", x: 1.0 / 3.0, y: 1.0 / 3.0)
}
...
public enum TransferFunction: Hashable, Sendable, Codable {
    ...
    public init(from decoder: any Decoder) throws {
        ...
        guard isValid else {
            throw DecodingError.dataCorruptedError(forKey: .kind, in: c, debugDescription: "Invalid transfer function")
        }
    }
...
public enum MixingMethod: Hashable, Sendable, Codable {
    ...
    private enum Kind: String, Codable { case perceptual, perceptualPolar, linearLight, encodedRGB, lab }

    public init(from decoder: any Decoder) throws {
        let c = try decoder.container(keyedBy: CodingKeys.self)
        switch try c.decode(Kind.self, forKey: .kind) {
        case .perceptual: self = .perceptual
        case .perceptualPolar: self = .perceptualPolar(huePath: try c.decode(HueInterpolationPath.self, forKey: .huePath))
...
public struct RGBColorProfile: Hashable, Sendable, Codable, Identifiable {
    ...
    public init(
        ...
        precondition(matrix.determinant.isFinite && abs(matrix.determinant) > 1e-18, "Profile matrix must be invertible")
    }
...
public extension RGBColorProfile {
    static let sRGB = RGBColorProfile(
        name: "sRGB",
        primaries: RGBPrimaries(
    ...
    static let displayP3 = RGBColorProfile(
        name: "Display P3",
        primaries: RGBPrimaries(
    ...
    static let adobeRGB = RGBColorProfile(
        name: "Adobe RGB",
        primaries: RGBPrimaries(
            red: Chromaticity(x: 0.64, y: 0.33),
    ...
    static let rec2020 = RGBColorProfile(
        name: "Rec. 2020",
        primaries: RGBPrimaries(
    ...
    static let proPhotoRGB = RGBColorProfile(
        name: "ProPhoto RGB",
        primaries: RGBPrimaries(
    ...
    static let acescg = RGBColorProfile(
        name: "ACEScg",
        primaries: RGBPrimaries(
    ...
    static let linearSRGB = RGBColorProfile(
        name: "Linear sRGB",
    ...
    static let linearDisplayP3 = RGBColorProfile(
...
extension RGBColorProfile {
    ...
    fileprivate var linearized: RGBColorProfile {
        ...
        return RGBColorProfile(name: "\(name) Linear", primaries: primaries, whitePoint: whitePoint, transferFunction: .linear)
    }
}
...
public func adapt(
    ...
    let mInv = m.inverse
    let sourceCone = m * Vector3(x: xyz.whitePoint.xyz.x, y: xyz.whitePoint.xyz.y, z: xyz.whitePoint.xyz.z)
    ...
    precondition(_PrismNumeric.isFinite(adapted.x, adapted.y, adapted.z), "Chromatic adaptation must preserve finiteness")
...
internal func _xyzD65ToOklab(_ xyz: XYZColor) -> OklabColor {
    ...
    let l = _PrismNumeric.cbrtSigned(0.8189330101 * xyz.x + 0.3618667424 * xyz.y - 0.1288597137 * xyz.z)
    let m = _PrismNumeric.cbrtSigned(0.0329845436 * xyz.x + 0.9293118715 * xyz.y + 0.0361456387 * xyz.z)
    let s = _PrismNumeric.cbrtSigned(0.0482003018 * xyz.x + 0.2643662691 * xyz.y + 0.6338517070 * xyz.z)
    return OklabColor(
...
public struct Color: Hashable, Sendable, Codable {
    ...
    public init(red: Double, green: Double, blue: Double, alpha: Double = 1.0, profile: RGBColorProfile = .sRGB) {
    ...
    public init(hex: String, profile: RGBColorProfile = .sRGB) throws {
        let trimmed = String(hex.drop(while: \.isWhitespace).reversed().drop(while: \.isWhitespace).reversed())
        let normalized = trimmed.hasPrefix("#") ? String(trimmed.dropFirst()) : trimmed
        guard !normalized.isEmpty else { throw ColorError.invalidHexString(hex) }
        guard normalized.unicodeScalars.allSatisfy({ "0123456789ABCDEFabcdef".unicodeScalars.contains($0) }) else {
        ...
        case 3:
            ...
            self.init(red: byte(r[...]), green: byte(g[...]), blue: byte(b[...]), alpha: 1.0, profile: profile)
        case 4:
            let chars = Array(normalized)
            let r = String([chars[0], chars[0]])
            let g = String([chars[1], chars[1]])
            ...
            self.init(red: byte(r[...]), green: byte(g[...]), blue: byte(b[...]), alpha: byte(a[...]), profile: profile)
        case 6:
            self.init(
    ...
    public init(from decoder: any Decoder) throws {
        ...
        guard version == 1 else {
            throw DecodingError.dataCorruptedError(forKey: .schemaVersion, in: c, debugDescription: "Unsupported Color schema version: \(version)")
        }
        ...
        if let name = try? c.decode(String.self, forKey: .profile) {
            guard let builtIn = RGBColorProfile.builtIn(named: name) else {
                throw DecodingError.dataCorruptedError(forKey: .profile, in: c, debugDescription: "Unknown built-in profile: \(name)")
            }
            profile = builtIn
        } else {
...
public extension Color {
    static let clear = Color(red: 0, green: 0, blue: 0, alpha: 0, profile: .sRGB)
    static let black = Color(red: 0, green: 0, blue: 0, alpha: 1, profile: .sRGB)
    static let white = Color(red: 1, green: 1, blue: 1, alpha: 1, profile: .sRGB)
    static let red = try! Self(hex: "#E05757")
    static let green = try! Self(hex: "#61C67B")
    static let blue = try! Self(hex: "#5BA3FF")
    static let yellow = try! Self(hex: "#EBB33C")
    static let magenta = try! Self(hex: "#B46EFF")
    static let cyan = try! Self(hex: "#56B6C2")
    static let gray = try! Self(hex: "#8C92AC")
}
...
extension Color {
    ...
    @inlinable
    internal static func _fromLinearRGB(_ linear: Vector3, alpha: Double, profile: RGBColorProfile) -> Color {
        Color(
    ...
    @inlinable
    internal static func _fromXYZPreservingGamut(_ xyz: XYZColor, alpha: Double, profile: RGBColorProfile) -> Color {
    ...
    @inlinable
    internal static func _fromOklab(_ lab: OklabColor, alpha: Double, profile: RGBColorProfile) -> Color {
        let xyz = _oklabToXYZD65(lab)
        return _fromXYZPreservingGamut(xyz, alpha: alpha, profile: profile)
    }

    @inlinable
    internal static func _fromOklch(_ lch: OklchColor, alpha: Double, profile: RGBColorProfile) -> Color {
    ...
    internal func _isInGamutEncoded(tolerance: Double = 1e-9) -> Bool {
        red >= -tolerance && red <= 1 + tolerance &&
        green >= -tolerance && green <= 1 + tolerance &&
        blue >= -tolerance && blue <= 1 + tolerance
    }

    internal func _compressChroma(to targetProfile: RGBColorProfile, preserveLightness: Bool) -> Color? {
        ...
        let achromatic = Color._fromOklch(OklchColor(l: _PrismNumeric.clamp(original.l, 0, 1), c: 0, h: original.h), alpha: alpha, profile: targetProfile)
    ...
    internal func _mappedPreservedColor(_ preserved: Color, from original: Color, to targetProfile: RGBColorProfile, policy: GamutMappingPolicy) -> Color {
        switch policy {
        case .preserve:
            return preserved
        ...
        case .compressLightness:
            return original._compressChroma(to: targetProfile, preserveLightness: false) ?? preserved._clippedChannels()
        case .compressPerceptual:
...
public extension Color {
    ...
    func xyz(adaptedTo whitePoint: ReferenceWhite, method: ChromaticAdaptationMethod = .bradford) -> XYZColor {
        adapt(xyz(), to: whitePoint, method: method)
    }

    func lab(whitePoint: ReferenceWhite = .d50) -> LabColor {
        _xyzToLab(xyz(), whitePoint: whitePoint)
    }

    func lch(whitePoint: ReferenceWhite = .d50) -> LChColor {
        _labToLCh(lab(whitePoint: whitePoint))
    }

    func oklab() -> OklabColor {
        let d65XYZ = xyz().whitePoint == .d65 ? xyz() : xyz(adaptedTo: .d65, method: .bradford)
        return _xyzD65ToOklab(d65XYZ)
    }

    func oklch() -> OklchColor {
        _oklabToOklch(oklab())
    }

    func converted(to profile: RGBColorProfile, gamutMapping: GamutMappingPolicy = .default) -> Color {
    ...
    func mapped(to profile: RGBColorProfile, policy: GamutMappingPolicy) -> Color {
        converted(to: profile, gamutMapping: policy)
    }

    func clamped(to profile: RGBColorProfile) -> Color {
        converted(to: profile, gamutMapping: .clip)
    }

    func isInGamut(for profile: RGBColorProfile, tolerance: Double = 1e-9) -> Bool {
...
internal func _deltaE2000(_ lhs: LabColor, _ rhs: LabColor) -> Double {
    let l1 = lhs.l, a1 = lhs.a, b1 = lhs.b
    let l2 = rhs.l, a2 = rhs.a, b2 = rhs.b
...
public extension Color {
    ...
    func deltaE(to other: Color, method: DeltaEMethod = .ok) -> Double {
        switch method {
        case .ok:
            let a = oklab()
            let b = other.oklab()
    ...
    func isApproximatelyEqual(to other: Color, deltaE tolerance: Double = 0.5, method: DeltaEMethod = .ok) -> Bool {
...
public extension Color {
    func mixed(with other: Color, amount: Double, method: MixingMethod = .perceptual) -> Color {
        interpolated(to: other, progress: amount, method: method)
    }

    func interpolated(to other: Color, progress: Double, method: MixingMethod = .perceptual) -> Color {
        ...
        case .perceptual:
            ...
            return Color._fromOklab(mixed, alpha: alpha, profile: self.profile).mapped(to: self.profile, policy: .compressPerceptual)

        case .perceptualPolar(let huePath):
            let a = self.oklch()
            let b = other.oklch()
            let hue = interpolateHue(
            ...
            return Color._fromOklch(mixed, alpha: alpha, profile: self.profile).mapped(to: self.profile, policy: .compressPerceptual)
        ...
        case .linearLight:
            ...
            let linear = Vector3(
                x: _PrismNumeric.lerp(lhs.profile.transferFunction.decode(lhs.red), rhs.profile.transferFunction.decode(rhs.red), t),
                y: _PrismNumeric.lerp(lhs.profile.transferFunction.decode(lhs.green), rhs.profile.transferFunction.decode(rhs.green), t),
                z: _PrismNumeric.lerp(lhs.profile.transferFunction.decode(lhs.blue), rhs.profile.transferFunction.decode(rhs.blue), t)
            )
        ...
        case .encodedRGB:
            ...
            return mixed.isInGamut(for: self.profile) ? mixed : mixed.mapped(to: self.profile, policy: .compressPerceptual)
        ...
        case .lab:
            ...
            return Color._fromLab(mixed, alpha: alpha, profile: self.profile).mapped(to: self.profile, policy: .compressPerceptual)
        }
    }
}
...
public extension Color {
    func lightened(by amount: Double) -> Color {
        ...
        return Color._fromOklch(result, alpha: alpha, profile: profile).mapped(to: profile, policy: .compressPerceptual)
    }
    ...
    func accessibleTextColor(light: Color = .white, dark: Color = .black) -> Color {
...
public extension Color {
    func composited(
        ...
        let outPre = Vector3(
            x: (1.0 - bg.alpha) * fg.alpha * sf.x + (1.0 - fg.alpha) * bg.alpha * sb.x + fg.alpha * bg.alpha * blended.x,
            y: (1.0 - bg.alpha) * fg.alpha * sf.y + (1.0 - fg.alpha) * bg.alpha * sb.y + fg.alpha * bg.alpha * blended.y,
            z: (1.0 - bg.alpha) * fg.alpha * sf.z + (1.0 - fg.alpha) * bg.alpha * sb.z + fg.alpha * bg.alpha * blended.z
        )
