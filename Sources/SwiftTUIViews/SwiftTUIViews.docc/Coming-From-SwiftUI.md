...
Start with the good news: the reflexes transfer, even where a signature
narrows.
...
on Linux, WASI, and Android. Strict, unsuppressed concurrency is visible in
the API — environment models must be `Sendable`, `Binding(get:set:)`
accessors are `@MainActor`, and some generic bounds add `Sendable` — because
frames finish on an off-main frame tail. The natural authoring shape
...
stream: complete key events, no down/up/repeat phases. Collections are
...
`Picker` options are scraped to labeled text. One stance, four surfaces:
structured value metadata gives deterministic terminal chrome without
resolving arbitrary label view trees.
...
and `PhaseAnimator` holds its first phase — and `CI=true` or a non-TTY
stdout imply it. Semantic style roles (`.foreground`, `.tint`, `.warning`,
...
Beyond the recorded gaps sits a larger class of bare absences — among the
high-traffic ones: `searchable`, `refreshable`, `contextMenu`,
