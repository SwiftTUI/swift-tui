...
How SwiftTUI apps launch through runner packages or live inside embedded host
packages.
...
- **Embedded host package** — retain ``HostedSceneSession`` values inside another app's lifecycle. `Platforms/SwiftUI` does this for native SwiftUI on Apple platforms; `Platforms/Web` does it for browser hosting on top of a WASI build.
- **WebHost runner and browser host** — import `SwiftTUIWebHost` for web-only
  ...
  support both terminal-native and `--web` launch. This package is intentionally
...
- <doc:Runtime>
- <doc:Vision>
- [Platform Integration Packages](https://github.com/adamz/swift-tui/blob/main/docs/HOST_PACKAGES.md)
