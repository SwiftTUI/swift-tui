...
- **Embedded host package** — retain ``HostedSceneSession`` values inside another app's lifecycle. `Platforms/SwiftUI` does this for native SwiftUI on Apple platforms; `Platforms/Web` and `GUI/XtermWebHost` do it for browser hosting on top of a WASI build.
