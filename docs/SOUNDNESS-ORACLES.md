...
| `state-capture-miss` — a bound capture's dead owner always recovers through the fire-time identity refresh | Capture rung in [`State.swift`](../Sources/SwiftTUIViews/State/State.swift), recorded only while `StateCaptureBindingConfiguration.isEnabled` | T-fail | Event | Trace scan and serialized snapshot delta | `StateCaptureBindingTests`, `SoundnessFailureChannelTests` | Committed removal legitimately falls through; deliberate-removal tests suppress tracing while proving the counter  <!-- oracle-map: state-capture-miss ; recordStateCaptureMissViolation ; stateCaptureMissViolationCount --> |
