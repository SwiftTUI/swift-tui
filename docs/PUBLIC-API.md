...
- **Protocol-backed style families today** are `ShapeStyle`, `ToolbarStyle`,
  ...
  `StepperStyle`, `MenuStyle`, `ControlGroupStyle`, `PromptStyle`,
  `FullScreenCoverStyle`, `PopoverStyle`, `PaletteStyle`, `ScrollViewStyle`,
  and `LinkStyle`.
- **Type-erased style storage** provides concrete values where environment or
  ...
  `AnySliderStyle`, `AnyStepperStyle`, `AnyMenuStyle`, `AnyControlGroupStyle`,
  `AnyPromptStyle`, `AnyFullScreenCoverStyle`, `AnyPopoverStyle`, `AnyPaletteStyle`,
  `AnyScrollViewStyle`, and `AnyLinkStyle`.
...
plumbing remains in `SwiftTUIPTYPrimitives`; terminal views ship from
[`swift-tui-terminal-view`](https://github.com/SwiftTUI/swift-tui-terminal-view). Showcase and
...
in `swift-tui-examples`. `SwiftTUITerminalView` from the separate `swift-tui-terminal-view` package
is the public terminal-embedding surface.
