...
- `ActionScopes/Panel.swift`, `ActionScopes/KeyCommandModifier.swift`, and `ActionScopes/PaletteCommandModifier.swift`: the `Panel` primitive with `.panel(id:)` / `.panel()` / `FocusContainment`, the `.keyCommand(...)` modifier for shallowest-wins keybindings, and the `.paletteCommand(...)` modifier plus `ActivePaletteCommand` + `EnvironmentValues.activePaletteCommands` for consumer-queryable palette surfaces. See [proposals/ACTION_SCOPES_AND_COMMANDS.md](proposals/ACTION_SCOPES_AND_COMMANDS.md); toolbar modifiers are still landing.
