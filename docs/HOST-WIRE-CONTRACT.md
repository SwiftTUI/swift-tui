...
| S3e | Complete: the browser/WASI shared input queue writes a control record larger than its remaining 64 KiB capacity as ordered chunks instead of rejecting it, so a large paste is delivered intact rather than dropped. A write that fits with nothing queued ahead of it still lands **synchronously**; only a write that cannot fit suspends and waits for capacity. Writes are **serialized**: a small write issued while a chunked one is suspended queues behind it rather than taking the fast path and landing inside the first record. Capacity waits are bounded by a caller-owned deadline, and a deadline failure surfaces as a runtime issue in the mount, not console-only. |
