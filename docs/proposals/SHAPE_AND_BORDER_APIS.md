...
> Audience: maintainers of swift-terminal-ui considering a revamp of shape, border,
> and fill APIs to make defaults attractive, escape hatches accessible, and
...
`BorderSet`. The enum stays internally for the rasterizer fast path but
isn't exposed.
...
"circle") they fall back to half-block approximations and dot the user
about it via a debug warning.
...
6. **`StrokeStyle` gains a `dashPattern: [Int]` field** (alternating
   on/off lengths) for callers who don't want to define a whole `BorderSet`
   just to get dashes. Renderer translates this to a multi-rune edge
   string at draw time.
...
```swift
Circle()
  .strokeBorder(.accent, style: .init(lineWidth: 1, lineVariant: .single))
...
1. The `.border(...)` modifier flips. Existing call sites that depend on
   inset behavior — there appear to be zero in this repo today, but a
   downstream user might — get the new outset behavior automatically.
   Their layouts grow by 2 cells in each affected dimension.
2. For users who *want* the old inset behavior, the explicit form is
   `.border(set: .innerHalfBlock)` or any custom `BorderSet` with
   `placement: .inset`.
3. `Shape.stroke` / `Shape.strokeBorder` keep their current semantics.
   Those are the SwiftUI-faithful escape hatches and should not change.
   The `.border` modifier is no longer implemented as
   `.overlay(Rectangle().strokeBorder(...))` — it goes through the new
   layout-aware path.
4. The closed `LineVariant` enum stays for backwards compatibility for
   one release cycle; `StrokeStyle.lineVariant` is deprecated in favor
   of `StrokeStyle.borderSet: BorderSet`. Internally the old enum maps
   to the new sets.
