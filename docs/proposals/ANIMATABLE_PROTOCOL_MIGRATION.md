...
**Tech Stack:** Swift 6.2+ (strict concurrency), Swift Testing (`import Testing`, `@Test`, `#expect`), Xcode 26+, swift-terminal-ui package (Core + View + TerminalUI module split).
...
- **Test file reorganization.** `Tests/TerminalUITests/AnimationControllerTests.swift` stays one file even though it's large (~2000 lines). Splitting it is a separate cleanup.
...
3. **No visible animation regressions.** A checkpoint of pre-existing animations (opacity fade, color tween, padding animation, offset tween, position tween, frame size, border color, border blend phase) is exercised at each phase boundary. Fixtures: `AnimationControllerPropertyTests` suite in `Tests/TerminalUITests/AnimationControllerTests.swift`.
...
- Modify: `Tests/TerminalUITests/*GradientRenderingTests.swift` (fixtures)
- Modify: `Tests/TerminalUITests/BorderGradientTests.swift` (fixtures)
...
**Files:**
- Modify: `Sources/TerminalUI/AnimationController.swift` (substantial rewrite — ~420 LOC replaced)
- Create: `Sources/TerminalUI/AnyAnimatable.swift`
- Modify: `Tests/TerminalUITests/AnimationControllerTests.swift` (update tests that reference `AnimatableProperty` / `AnimatableValue`)

### Task 3.1: `AnyAnimatable` type-erased wrapper

- [ ] **Step 1: Write the failing tests**

Extend `Tests/TerminalUITests/AnimationControllerTests.swift` with a new test block (place it after the `FireCounter` helper, inside the same test file):
...
In `Sources/TerminalUI/AnimationController.swift`, *replace* the existing `AnimatableProperty` enum (`:12-28`) and `AnimatableValue` enum (`:32-36`) with:
...
Still in `Sources/TerminalUI/AnimationController.swift`, replace the `AnimatableSnapshot` struct (`:39-144`) with:
...
```bash
grep -n "AnimatableProperty\|AnimatableValue" Sources/TerminalUI/AnimationController.swift
```
...
```bash
grep -n "AnimatableProperty\|AnimatableValue" Tests/TerminalUITests/AnimationControllerTests.swift
```
...
git add \
  ...
  Sources/TerminalUI/AnyAnimatable.swift \
...
**Files:**
- Modify: `Sources/TerminalUI/AnimationController.swift`
- Modify: `Sources/TerminalUI/RunLoop+Rendering.swift`
- Modify: `Tests/TerminalUITests/AnimationControllerTests.swift` (new unit tests for the unified map; delete tests pinning the old separate-maps behavior)
...
In `Sources/TerminalUI/AnimationController.swift`, replace the three parallel structs (`ActiveAnimation`, `InsertionOffsetAnimation`, `MatchedGeometryAnimation`) with a single struct that carries a kind discriminator:
...
In `Sources/TerminalUI/AnimationController.swift`, change `AnimationTickResult`:
...
In `Sources/TerminalUI/RunLoop+Rendering.swift` at the animation-tick wake-up block (`:171-198`), replace the hacky `isIdentityAgnosticTick` bypass (added by commit `9d6a87d`) with the clean semantics:
...
```bash
grep -n "dominantActiveRequest" Sources/TerminalUI/
```
...
In `Sources/TerminalUI/RunLoop+Rendering.swift`, at `:288-291`:
...
In `Sources/TerminalUI/AnimationController.swift`, remove `dominantActiveRequest()` and any supporting state it used.
...
- Create: `Tests/TerminalUITests/GradientAnimationIntegrationTests.swift`
...
git add \
  ...
  Tests/TerminalUITests/GradientAnimationIntegrationTests.swift
