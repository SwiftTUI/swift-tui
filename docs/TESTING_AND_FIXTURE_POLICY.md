...
- When paint-path work changes, prefer explicit write-shape assertions
  (`linesTouched`, `cellsChanged`, `FrameArtifacts.presentationDamage`,
  `FrameDiagnostics.presentation.damage`, emitted cursor/script shape,
  synchronized framing, graphics replay scope, edit-op lowering) over broad
  "smaller than before" checks alone.
