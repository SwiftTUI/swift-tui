...
# ADR-0010: Crash guard lives in the CLI runner, not in TerminalUI
...
(`Runners/TerminalUICLI`), not in the root `TerminalUI` library.
...
Accepted. The crash guard ships in `TerminalUICLI`'s
`SceneRuntime`. The root `TerminalUI` library has no signal-handling
code paths.
...
- WASI builds (which have no signals) and browser hosts (which don't
  own signal handlers) link `TerminalUI` cleanly without paying for
...
- Embedded SwiftUI / browser hosts that *do* own a real tty can
  ...
  `UnixSignals` package without coordinating with TerminalUI's
  internals.

**Foreclosed:**

- A consumer using `TerminalUI` directly without a runner does not
