...
| `CommitPlan` | Ordered lifecycle and handler-installation work | The already-derived `SemanticSnapshot` for runtime consumers | Commit is the main-actor side-effect boundary for lifecycle, task, registration, focus, presentation, and animation effects. Prepared frame heads use draft/discard mechanics for runtime internals, but the runtime no longer exposes a declared frame-head rollback-effect set. Preview commit is checkpoint-restored. |
