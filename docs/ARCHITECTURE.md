...
| `CommitPlan` | Ordered lifecycle and handler-installation work | The already-derived `SemanticSnapshot` for runtime consumers | Commit is the main-actor side-effect plan for lifecycle, task, registration, focus, and presentation effects that can already be deferred. The runtime frame head still declares mutable `viewGraph`, `frameState`, `presentationPortalState`, `observationBridge`, and `animationController` effects; narrowing those toward commit is tracked in the [Finding 4 plan](plans/2026-05-17-008-finding-4-resolve-effect-narrowing-plan.md). Preview commit is checkpoint-restored. |
