    ...
    public static func serializeQueryParameters(_ parameters: [String: Any]) -> String {
        let components = parameters.compactMap { item -> String? in
            ...
            if let values = value as? [Any] {
                ...
                return "\(snakeKey)=\(serialized.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? serialized)"
            }

            guard let raw = paramsValueString(value) else {
                return nil
            }
            return "\(snakeKey)=\(raw.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? raw)"
        }
