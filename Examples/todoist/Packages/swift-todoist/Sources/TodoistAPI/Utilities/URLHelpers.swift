...
public enum URLHelpers {
    ...
    private static func slugify(_ value: String) -> String {
        ...
        let asciiOnly = normalized.replacingOccurrences(of: "[^\\x20-\\x7E]", with: "", options: .regularExpression)
        let valid = asciiOnly.lowercased().replacingOccurrences(of: "[^\\w\\s-]", with: "", options: .regularExpression)
        let hyphenated = valid
