...
struct AuthenticationAndUploadsTests {
    ...
    func authorizationURLMatchesTodoistOAuthSemantics() throws {
        ...
        #expect(
            url == "https://todoist.com/oauth/authorize?client_id=client-123&scope=data:read,task:add&state=state-abc"
        )
    }
    ...
    func revokeAndMigratePersonalTokenKeepCompatibility() async throws {
        ...
        #expect(revokeRequest.value(forHTTPHeaderField: "Authorization") == "Basic Y2xpZW50OnNlY3JldA==")
        ...
        let migrateTransport = MockTransport(actions: [.response(try jsonResponse([
        ...
        #expect(migrateRequest.url.absoluteString == "https://api.todoist.com/api/v1/access_tokens/migrate_personal_token")
    ...
    func uploadBuildsMultipartRequestAndForwardsRequestID() async throws {
        let transport = MockTransport(actions: [.response(try jsonResponse(attachmentJSON(defaultComment.fileAttachment!)))])
    ...
    func viewAttachmentEnforcesTodoistDomainsAndExposesReadableResponses() async throws {
        let transport = MockTransport(actions: [.response(textResponse("hello attachment", headers: ["content-type": "text/plain"]))])
        let client = TodoistClient(authToken: "secret-token", transport: transport)

        let response = try await client.uploads.viewAttachment(url: "https://files.todoist.com/user_upload/v2/123/file.txt")
        #expect(response.ok)
