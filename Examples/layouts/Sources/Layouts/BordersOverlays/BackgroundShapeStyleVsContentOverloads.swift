...
public struct BackgroundShapeStyleVsContentOverloads: View {
  ...
  public var body: some View {
    VStack(alignment: .leading, spacing: 1) {
      Text("Background ShapeStyle vs Content overloads").foregroundStyle(.muted)
      HStack(alignment: .top, spacing: 3) {
        VStack(alignment: .leading, spacing: 0) {
          Text(".background(.red):").foregroundStyle(.muted)
          Text("hello")
            .frame(width: 10, height: 2)
            .background(Color.red)
        }
        VStack(alignment: .leading, spacing: 0) {
          Text(".background { Rectangle } :").foregroundStyle(.muted)
          Text("hello")
            .frame(width: 10, height: 2)
            .background {
