...
class WasmSceneRuntime extends WebTUISceneRuntime {
  ...
  constructor(
    ...
    try {
      ...
      console.error("[XtermWebExample] failed to create shared stdin queue", error);
    }
    ...
    super({
      ...
      onInput: (chunk) => {
        try {
          ...
          console.error("[XtermWebExample] failed to enqueue terminal input", error);
        }
      },
    });
  ...
  override async mount(): Promise<void> {
    ...
    if (!this.inputQueue || !this.inputWriter || !this.bridge) {
      this.writeOutput(
        "\r\nXtermWebExample requires SharedArrayBuffer-backed stdin. Serve the app with COOP/COEP headers.\r\n"
      );
      return;
    }
    ...
    this.worker.addEventListener("error", (event) => {
      this.bridge?.stderr.write(
        `\nXtermWebExample worker failed: ${event.message || "unknown worker error"}\n`
      );
    });
  ...
  private handleWorkerMessage(
    ...
    case "exit":
      ...
      break;
    case "error":
      this.bridge?.stderr.write(`\nFailed to start XtermWebExample: ${message.message}\n`);
      break;
    }
  }
}
