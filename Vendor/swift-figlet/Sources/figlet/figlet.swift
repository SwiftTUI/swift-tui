...
struct FigletMain {
    static func main() {
        do {
            let exitCode = try run(arguments: Array(CommandLine.arguments.dropFirst()))
    ...
    private static func run(arguments: [String]) throws -> Int32 {
        ...
        if options.listFonts {
            let output = Figlet.availableFonts().joined(separator: "\n") + "\n"
            FileHandle.standardOutput.write(Data(output.utf8))
            return 0
        }

        let font = try FigletFont(named: options.font)
