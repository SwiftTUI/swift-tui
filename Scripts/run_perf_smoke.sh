...
output_file=$(mktemp "${TMPDIR:-/tmp}/termui-perf-smoke.XXXXXX")
trap 'rm -f "$output_file"' EXIT HUP INT TERM

if swiftly run swift run --package-path Tools/TermUIPerf -c release termui-perf run \
  --scenario gallery-animation-click \
  --modes sync,async \
  --iterations 1 \
  --configuration release >"$output_file" 2>&1
then
  cat "$output_file"
else
  status=$?
...
if [ -n "$base_run" ] && [ -n "$candidate_run" ] \
  && [ -f "$base_run/summary.json" ] && [ -f "$candidate_run/summary.json" ]; then
  if swiftly run swift run --package-path Tools/TermUIPerf -c release termui-perf compare \
    "$base_run" \
    "$candidate_run" > .perf/runs/latest-compare.txt 2>&1
  then
    cat .perf/runs/latest-compare.txt
  else
    status=$?
    cat .perf/runs/latest-compare.txt >&2
    exit "$status"
  fi
else
  echo "error: expected two complete run directories for sync/async comparison" >&2
  exit 1
fi
