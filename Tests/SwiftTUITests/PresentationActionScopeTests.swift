// Excluded from Windows builds (Windows plan, Stage 6 item 3, recorded
// blocker): the Swift 6.3.3 aarch64-windows frontend (+Asserts) aborts
// (0x80000003) in IRGen while "mangling type for debugger type %27some View%27"
// for modalFocusFixture(kind:isPresented:) — the opaque-return debug-info
// mangle, not our source (internal vs fileprivate made no difference).
// Re-admit when the toolchain moves; minimize + report upstream if it
// survives the next toolchain bump.
#if !os(Windows)
