...
struct TabViewLifecycleTests {
  ...
  // NOTE: the runtime "leaving an active geometry-backed tab cancels its task"
  // regression lives in `TabAutonomousTaskRuntimeTests`. It asserts the
  // observable runtime fact (`lifecycleCoordinator.activeTaskCount` → 0) on the
  // synchronous run-loop path. A `DefaultRenderer().render()` snapshot here can
  // only inspect a `.taskCancel` *plan entry* — the snapshot renderer never
  // starts or cancels real tasks — and tripped the latent TabView-snapshot
  // memory-corruption crash (`SwiftTUI/swift-tui#12`).
