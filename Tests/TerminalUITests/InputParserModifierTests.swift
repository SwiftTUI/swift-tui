...
struct InputParserModifierTests {
  ...
  func ctrlA() {
    ...
    #expect(events == [.key(KeyPress(.character("a"), modifiers: .control))])
  }
  ...
  func ctrlS() {
    ...
    #expect(events == [.key(KeyPress(.character("s"), modifiers: .control))])
  }
  ...
  func ctrlZ() {
    ...
    #expect(events == [.key(KeyPress(.character("z"), modifiers: .control))])
  }

  @Test("Ctrl+C emits ctrlC case with control modifier")
  func ctrlC() {
    var parser = TerminalInputParser()
    let events = parser.feed([0x03])
    #expect(events == [.key(KeyPress(.ctrlC, modifiers: .control))])
  }
  ...
  @Test("Enter (0x0D) emits enter without modifiers")
  func enter() {
    var parser = TerminalInputParser()
    let events = parser.feed([0x0D])
    #expect(events == [.key(KeyPress(.enter))])
  }

  // MARK: - Alt+letter

  @Test("Alt+x emits character 'x' with option modifier")
  func altX() {
    var parser = TerminalInputParser()
    let events = parser.feed([0x1B, 0x78])
    #expect(events == [.key(KeyPress(.character("x"), modifiers: .option))])
  }

  @Test("Alt+Space emits space with option modifier")
  func altSpace() {
    var parser = TerminalInputParser()
    let events = parser.feed([0x1B, 0x20])
    #expect(events == [.key(KeyPress(.space, modifiers: .option))])
  }
  ...
  func ctrlUp() {
    ...
    #expect(events == [.key(KeyPress(.arrowUp, modifiers: .control))])
  }
  ...
  @Test("Alt+Right emits arrowRight with option modifier")
  func altRight() {
    // ESC [ 1 ; 3 C = Alt+Right (modifier param 3 = 1 + alt bitmask 2)
    var parser = TerminalInputParser()
    let events = parser.feed([0x1B, 0x5B, 0x31, 0x3B, 0x33, 0x43])
    #expect(events == [.key(KeyPress(.arrowRight, modifiers: .option))])
  }
  ...
  func ctrlShiftLeft() {
    ...
    #expect(events == [.key(KeyPress(.arrowLeft, modifiers: [.control, .shift]))])
  }

  // MARK: - Shift+Tab

  @Test("Shift+Tab emits shiftTab with shift modifier")
  func shiftTab() {
    // ESC [ Z = Shift+Tab
    var parser = TerminalInputParser()
    let events = parser.feed([0x1B, 0x5B, 0x5A])
    #expect(events == [.key(KeyPress(.shiftTab, modifiers: .shift))])
  }
