...
// This whole suite introspects FocusState's wrapper storage through
// `debugStorageObject`, which is `#if DEBUG` in SwiftTUIViews — the storage
// identity it asserts on is not observable in release configuration at all.
// Guarding the file keeps `swift test -c release` (the release-soundness lane)
// compiling; leaving it unguarded fails the whole SwiftTUIViewsTests module and
// costs every release-config test, not just these.
#if DEBUG
  ...
  struct FocusStateOwnerLifetimeTests {
    ...
    func equalRuntimeApplyDoesNotInvalidateOwner() throws {
      ...
      resolve(
        FocusOwnerLifetimeReadingProbe(capture: capture), identity: ownerIdentity, graph: graph)
      let binding = try #require(capture.binding)
      let owner = try #require(graph.nodeForIdentity(ownerIdentity))
      let invalidator = FocusOwnerLifetimeInvalidator()
      owner.invalidator = invalidator
